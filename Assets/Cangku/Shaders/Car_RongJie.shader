Shader "Custom/Car_RongJie"
{
	Properties
	{
		...
		_NoiseInfluence("Noise Influence", Range( -200 , 200)) = 50
		...
		_XandYoffset("X and Y offset", Vector) = (0,0,0,0)
		[Toggle]_EMI_on("EMI_on", Float) = 0
		_EMI("EMI", 2D) = "white" {}
		_EMI_I("EMI_I", Float) = 1
		_EMI_color("EMI_color", Color) = (1,1,1,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			UNITY_INSTANCING_BUFFER_START(KnifeHexMetallic)
				...
				UNITY_DEFINE_INSTANCED_PROP(float4, _EMI_ST)
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float temp_output_134_0 = ( break14.z + ( simplePerlin2D132 * (( _ToggleSwitch0 )?( ( 1.0 - _NoiseInfluence ) ):( _NoiseInfluence )) ) );
				...
				float3 final_v_offset195 = ( offset_direction102 * _XandYoffset * clip_1_no_offset227 );
				
				o.ase_texcoord9.xyz = vertexToFrag52;
						 ...
						 ) : SV_Target
			{
				...
				float4x4 _ClipQuadMatrix_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_ClipQuadMatrix);
				float3 vertexToFrag52 = IN.ase_texcoord9.xyz;
				float4 appendResult12 = (float4(vertexToFrag52 , 1.0));
				float4 break14 = mul( _ClipQuadMatrix_Instance, appendResult12 );
				float2 appendResult133 = (float2(break14.x , break14.y));
				float simplePerlin2D132 = snoise( appendResult133*_NoiseScale );
				...
				float4 triplanar4 = TriplanarSampling4( _HexPattern, IN.ase_texcoord10.xyz, IN.ase_normal, _Falloff, temp_cast_2, 1.0, 0 );
				...
				float4 _EMI_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_EMI_ST);
				float2 uv_EMI = IN.ase_texcoord8.xy * _EMI_ST_Instance.xy + _EMI_ST_Instance.zw;
				
				float4 _MetallicSmoothness_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_MetallicSmoothness_ST);
				float2 uv_MetallicSmoothness = IN.ase_texcoord8.xy * _MetallicSmoothness_ST_Instance.xy + _MetallicSmoothness_ST_Instance.zw;
				...
				float final_ao197 = ( tex2DNode186.g * _AOStrength );
				...
				float3 BaseColor = final_color207.rgb;
				float3 Normal = final_normal205;
				float3 Emission = ( float4( final_emission203 , 0.0 ) + (( _EMI_on )?( ( float4( tex2D( _EMI, uv_EMI ).rgb , 0.0 ) * _EMI_color * _EMI_I ) ):( float4( 0,0,0,0 ) )) ).rgb;
				float3 Specular = 0.5;
		...
		Pass
		{
						 ...
						 ) : SV_TARGET
			{
				...
				float4 triplanar4 = TriplanarSampling4( _HexPattern, IN.ase_texcoord4.xyz, IN.ase_normal, _Falloff, temp_cast_0, 1.0, 0 );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float4 triplanar4 = TriplanarSampling4( _HexPattern, IN.ase_texcoord7.xyz, IN.ase_normal, _Falloff, temp_cast_2, 1.0, 0 );
				float hex_offset175 = (0.0 + (triplanar4.y - 0.0) * (-1.0 - 0.0) / (1.0 - 0.0));
				float temp_output_179_0 = ( hex_offset175 * _HexMaxOffset );
				float smoothstepResult18 = smoothstep( _DistanceMin , _DistanceMax , ( temp_output_134_0 + temp_output_179_0 ));
				float clip_1106 = saturate( smoothstepResult18 );
				...
				float3 final_emission203 = clampResult151;
				float4 _EMI_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_EMI_ST);
				float2 uv_EMI = IN.ase_texcoord4.xy * _EMI_ST_Instance.xy + _EMI_ST_Instance.zw;
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				o.ase_texcoord7.xyz = vertexToFrag52;
						 ...
						 )
			{
				...
				float4 _NormalMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_NormalMap_ST);
				float2 uv_NormalMap = IN.ase_texcoord5.xy * _NormalMap_ST_Instance.xy + _NormalMap_ST_Instance.zw;
				float3 unpack46 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap ), _NormalScale );
				unpack46.z = lerp( 1, unpack46.z, saturate(_NormalScale) );
				float3 final_normal205 = unpack46;
				
				float2 temp_cast_0 = (_Tiling).xx;
				float4 triplanar4 = TriplanarSampling4( _HexPattern, IN.ase_texcoord6.xyz, IN.ase_normal, _Falloff, temp_cast_0, 1.0, 0 );
				float hex98 = triplanar4.x;
				float4x4 _ClipQuadMatrix_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_ClipQuadMatrix);
				float3 vertexToFrag52 = IN.ase_texcoord7.xyz;
				float4 appendResult12 = (float4(vertexToFrag52 , 1.0));
				float4 break14 = mul( _ClipQuadMatrix_Instance, appendResult12 );
				float2 appendResult133 = (float2(break14.x , break14.y));
				float simplePerlin2D132 = snoise( appendResult133*_NoiseScale );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				float4 triplanar4 = TriplanarSampling4( _HexPattern, IN.ase_texcoord2.xyz, IN.ase_normal, _Falloff, temp_cast_0, 1.0, 0 );
				float hex98 = triplanar4.x;
				float4x4 _ClipQuadMatrix_Instance = UNITY_ACCESS_INSTANCED_PROP(KnifeHexMetallic,_ClipQuadMatrix);
				...
				surfaceDescription.Alpha = final_opacity193;
