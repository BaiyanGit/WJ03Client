...
namespace RainbowArt.CleanFlatUI
{
    ...
    public class PopupMenu : MonoBehaviour, IPointerDownHandler
    {
        ...
        public void ShowPopupMenu(Vector3 position, float width, float height, bool isFalse = false)
        {
            if (options.Count > 0)
            {
                ...
                UpdatePosition(position, width, height, isFalse);
                CreateClickBlocker();
                PlayAnimation(true);
            }
        }
        ...
        void UpdatePosition(Vector3 position, float uiWidth, float uiHeight, bool posFlase = false)
        {
