...
public class CRC16Standard 
{
    ...
    public static byte[] CrcCalc(byte[] data,bool order = false)
    {
        int crc = 0xFFFF;
    ...
    public static byte[] CrcCalc(byte[] data,int offset,int length,bool order = false)
    {
        byte[] Tdata = data.Skip(offset).Take(length).ToArray();
        return CrcCalc(Tdata,order);
    }
}
