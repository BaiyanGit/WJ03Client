...
namespace Wx.Runtime
{
    ...
    public class ReferenceCollector : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        {
            return !dict.TryGetValue(key, out var dictGo) ? null : (dictGo as GameObject)?.GetComponent<T>();
        }
        ...
        public void AddCollection(string key, Object obj)
        {
            int i;
            ...
            {
                ReferenceCollectorData item = new()
                {
                    key = key,
                    gameObject = obj
                };
