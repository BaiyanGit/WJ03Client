        ...
        public void Init(DataTable sheet, string excelPath)
        {
            ...
            for (int i = ExcelConvertPathSetting.EXCEL_ROW_INDEX_Content_Start; i < realRowCount; i++)
            {
                ...
                for (int j = 0; j < realColumns.Count; j++)
                {
                    ...
                    {
                        WLog.Warning($"值为空,title:【{propTitle}】, row:【{i + 1}】, path:" + excelPath);
                    ...
                    if (propType.Contains("[]"))
                    {
                        ...
                        {
                            WLog.Error($"array error,数组错误 title:【{propTitle}】, row:【{i + 1}】, value:【{propValue}】, path:【{excelPath}】");
                        }
                    }

                    if (dic.ContainsKey(propTitle))
                    {
                        WLog.Error("title repeat:【" + propTitle + "】title重复定义, path: " + excelPath);
                    }
                    else
                    {
