...
using PrefabStage = UnityEditor.SceneManagement.Stage;
#endif
            ...
            void addTab()
            {
                ...
                void defaultTabs()
                {
                    ...
                    foreach (var tabInfo in defaultTabList)
                        ...
                        {
                            tabInfo.CreateWindow(GetDockArea(EditorWindow.mouseOverWindow), false);
                ...
                void saveCurrentTab()
                {
                    var menuItemName = EditorWindow.mouseOverWindow.titleContent.text.Replace("/", " \u2215 ").Trim(' ');
                    ...
                    {
                        customTabList.Add(new TabInfo(EditorWindow.mouseOverWindow));
                        saveSavedTabsList();
                        EditorApplication.delayCall += rememberClickPosition;
        ...
        class TabInfo
        {
            public EditorWindow CreateWindow(object dockArea = null, bool atOriginalTabIndex = true)
            {
                ...
                void setupBrowser()
                {
                    ...
                    void setSavedGridSize()
                    {
                        ...
                        window.GetFieldValue("m_ListArea")?.SetMemberValue("gridSize", gridSize);
                ...
                void setupPropertyEditor()
                {
                    ...
                    var lockTo = lockedPrefabAssetObject ?? globalId.GetObject(); // globalId api doesn't work for prefab asset objects, so we use direct object reference in such cases 
            ...
            public TabInfo(EditorWindow window)
            {
                ...
                if (isBrowser)
                {
                    ...
                    folderGuid = AssetDatabase.AssetPathToGUID(window.InvokeMethod<string>("GetActiveFolderPath"));
                    gridSize = window.GetFieldValue<int>("m_StartGridSize");
                }
